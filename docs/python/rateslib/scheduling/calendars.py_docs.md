...
- `get_calendar()`
- `_parse_str_calendar()`
- `_parse_str_calendar_no_associated()`
- `_parse_str_calendar_with_associated()`
...
> Returns a calendar object either from an available set or a user defined input.
    ...
    named : bool
        If the calendar is more complex than a pre-existing single name calendar, then
        this argument determines if a :class:`~rateslib.scheduling.NamedCal` objec
